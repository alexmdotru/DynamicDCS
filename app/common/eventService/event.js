			...
			_.forEach(sortedEvents, function (event) {
				...
				if (curiPlayer || curtPlayer) {
					if (curiPlayer) {
						...
						scoreMath = _.get(eCtrl, ['curScore', curiPlayer], 0) + _.get(event, 'score', 0);
						if (scoreMath < 0) {
							scoreMath = 0;
						}
						_.set(eCtrl, ['curScore', curiPlayer], scoreMath);
					} else {
					...
					_.set(event, 'y',_.get(eCtrl, ['curScore', curiPlayer], 0));
